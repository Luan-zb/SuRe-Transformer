...
class PerceiverAttention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None):
        ...
        #print("q.shape,k.shape,v.shape:", q.shape, k.shape, v.shape)    #torch.Size([1, 256, 64]) torch.Size([1, 253, 64]) torch.Size([1, 253, 64])

        sim = einsum('b i d, b j d -> b i j', q, k) * self.scale
        # if exists(mask):
        #     # mask = rearrange(mask, 'b ... -> b (...)')
        #     # # mask = rearrange(mask, 'b n d -> b d n')
        #     # print("mask.shape", mask.shape)
        #     # max_neg_value = -torch.finfo(sim.dtype).max
        #     # mask = repeat(mask, 'b j -> (b h) () j', h=h).bool()
            

        #     mask = rearrange(mask, 'b j k -> b k j')  # Rearrange (1, 253, 3) to (1, 3, 253)
        #     max_neg_value = -torch.finfo(sim.dtype).max
        #     mask = repeat(mask, 'b k j -> (b h) k j', h=h).bool() 
