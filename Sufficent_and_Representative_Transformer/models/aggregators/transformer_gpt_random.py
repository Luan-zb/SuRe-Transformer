...
from models.aggregators.model_utils_gpt_random import Attention, FeedForward, PreNorm
...
class TransformerBlocks(nn.Module):
    ...
    def forward(self, x, distances_matrix=None, register_hook=False):
        for attn, ff in self.layers:
            x = attn(x, distances_matrix =distances_matrix, register_hook=register_hook) + x
            x = ff( x, distances_matrix = distances_matrix) + x
        return x


class Transformer_gpt_random(BaseAggregator):
    def __init__(
        self,
        *,
        num_classes,
    ...
    def forward(self, x, coords=None, distances_matrix=None, register_hook=False):
        ...
        x = self.dropout(x)
        x = self.transformer(x, distances_matrix=distances_matrix, register_hook=register_hook)
        x = x.mean(dim=1) if self.pool == 'mean' else x[:, 0]

        return self.mlp_head(self.norm(x))


transformer = Transformer_gpt_random(num_classes=2)
