...
def process_data(h5_Dir,slide_csv,clin_csv):
    ...
    hrd_scores = hrd_hrp_label_df['HRD_Score'].to_numpy()
    # 计算上下三分位数
    ...
    hrd_hrp_label_df['label']  = np.where(hrd_hrp_label_df['HRD_Score'] <= q1, 'HRP', np.where(hrd_hrp_label_df['HRD_Score'] >= q2, 'HRD', 'Uncertain'))
    hrd_hrp_label_df = hrd_hrp_label_df[(hrd_hrp_label_df['label'] == 'HRP') | (hrd_hrp_label_df['label'] == 'HRD')]
...
if __name__=='__main__':
    ...
    #r'/work1/lhj/project/HrdTransformer/get_feature_multi/features_argumentated/200n_600s'
    clin_csv="clinic.csv"
